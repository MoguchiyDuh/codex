...
Dijkstra explores the graph in concentric circles around the source — wasteful when you only need a path to *one* specific target. A* biases the search toward the goal using a heuristic $h(v)$ that estimates the remaining cost from $v$ to the target.
